...
from SinGAN.imresize import imresize, videoresize
import os
import random
...
def convert_image_np_4D(inp):
    inp = denorm(inp)
    inp = move_to_cpu(inp[:,:,:,:])
#    inp = inp.numpy().transpose((0,2,3,1)) #TODO merge with video call
#    images = images.transpose(1,0,2,3)

    inp = inp.numpy()
#    inp = inp.transpose((3,0,1,2))
...
#       commented out since scaling is always 1, may have to update
...
def upsampling3d(im,sw,sx,sy):
    m = nn.Upsample(size=[round(sw),round(sx),round(sy)],mode='bilinear',align_corners=True)
    return m(im)
...
def read_image_dir(dir,opt):
    ...
    x = x[:,0:3,:,:]
    return x
def np2torchwithoutmove(x,opt):
    if opt.nc_im == 3:
        x = x[:,:,:,None]
        x = x.transpose((3, 2, 0, 1))/255
    #       Implementation not needed since we just recall and append
    #        x = x[:,:,:,:,None]
    #        x = x.transpose(4, 3, 2, 0, 1)
    else:
        x = color.rgb2gray(x)
        x = x[:,:,None,None]
...
def adjust_scales2image4D(real_,opt):
    opt.scaleacross4D = False
    if(opt.scaleacross4D):
        #TODO
        pass
    else:
        real = []
...
#            print("i shape" + str(i.shape))
            adjust_scales2image(i,opt)
#            print("i shape" + str(i.shape))

#            sys.exit(1)
            real.append(adjust_scales2image(i,opt))
#            print("PASS")
    return real
def adjust_scales2image(real_,opt):
    #opt.num_scales = int((math.log(math.pow(opt.min_size / (real.shape[2]), 1), opt.scale_factor_init))) + 1
#    print(real_.shape[0])
#    print(real_.shape[1])
#    print(real_.shape[2])
#    print(real_.shape[3])
...
def creat_reals_pyramid(real,reals,opt):
    ...
    if(opt.dimensions == 4):
        ...
        for i in range(0,opt.stop_scale+1,1):
            ...
            curr_real = videoresize(real,scale,opt)
...
def generate_in2coarsest(reals,scale_w,scale_v,scale_h,opt):
    real = reals[opt.gen_start_scale]
    real_down = upsampling3d(real, scale_w * real.shape[2], scale_v * real.shape[3], scale_v * real.shape[4])
    if opt.gen_start_scale == 0:
        #Wait Why. Wouldn't it be better to just do real.shape *scaling
        in_s = torch.full(real_down.shape, 0, device=opt.device)
    else: #if n!=0
        in_s = upsampling3d(real_down, real_down.shape[2], real_down.shape[3])
    return in_s
...
#Andreas Video to Image
def VideotoImage(opt):
    name = opt.input_name
    vidcap = cv2.VideoCapture('Input/Videos/%s.mp4' % name)
    success,image = vidcap.read()
    count = 0
    while success and count < opt.frame_cap:
...
#        print('Read a new frame: ', success)
    ...
    return count
def VideoSave(loc,name,data):
    #nothing is easy. data = tensor[4,x,x,x]
    images = convert_image_np_4D(data)
    #convert c,d,w,h to d,c,w,h so we get cwh through iterations
    images = images.transpose(1,2,3,0)
    for e,i in enumerate(images):
#        print(i.shape)
        plt.imsave('%s/%s%d.png' %  (loc,name,e), i, vmin=0, vmax=1)
