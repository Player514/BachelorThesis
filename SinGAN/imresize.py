...
def videoresize(im,scale,opt):
    imgs = []
    for sim in im:
        sim = sim[None,:,:,:]
        nsim = torch2uint8(sim)
        nsim = imresize_in(nsim, scale_factor=scale)
        imgs.append(nsim)
    imgs = np2torch4D(imgs,opt)
#    print(imgs.shape)
    return imgs
...
def fix_scale_and_size(input_shape, output_shape, scale_factor):
    ...
    #print(input_shape, output_shape, scale_factor)
